...
package net.retrocade.sfxr {
     ...
     *
     * File modified by Maurycy Zarzycki
     *
     * @author Thomas Vian
     */
    ...
    public class SfxrSynth {
        ...
        /**
         * Copied properties for mutation base
         */
        ...
        /**
         * 1 / _envelopeLength0 (for quick calculations)
         */
        private var _envelopeOverLength0:Number;

        /**
         * 1 / _envelopeLength1 (for quick calculations)
         */
        private var _envelopeOverLength1:Number;
        /**
         * 1 / _envelopeLength2 (for quick calculations)
         */
        ...
        /**
         * Maximum period before sound stops (from minFrequency)
         */
        ...
        /**
         * Phase through the vibrato sine wave
         */
        ...
        /**
         * Amount to change the note by
         */
        private var _changeAmount:Number;
        /**
         * Counter for the note change
         */
        private var _changeTime:int;
        /**
         * Once the time reaches this limit, the note changes
         */
        private var _changeLimit:int;
        /**
         * Offset of center switching point in the square wave
         */
        private var _squareDuty:Number;
        /**
         * Amount to change the duty by
         */
        ...
        /**
         * Once the time reaches this limit, some of the variables are reset
         */
        ...
        /**
         * Buffer of net.retrocade.random values used to generate noise
         */
        ...
        /**
         * Number of samples added to the buffer sample
         */
        private var _sampleCount:uint;
        /**
         * Another supersample used to create a 22050Hz wave
         */
        private var _bufferSample:Number;
        /**
         *
         */
        private var _playMutated:Boolean = false;
        ...
        public function play():void {
            ...
            _soundTransform.volume = RetrocamelSoundManager.soundVolume;
        ...
        public function playMutated(mutationAmount:Number = 0.05, mutationsNum:uint = 15):void {
            ...
            if (_cachingMutation != -1) {
                ...
                var r:uint = (_cachedMutations.length - 1) * Math.random() | 0;
                _waveData = _cachedMutations[r];
                _waveData.position = 0;
                _waveDataLength = _waveData.length;
                _waveDataBytes = 24576;
                _waveDataPos = 0;
            ...
            if (!_sound) {
                (_sound = new Sound()).addEventListener(SampleDataEvent.SAMPLE_DATA, onSampleData);
            }
        ...
        public function cacheSound(callback:Function = null, maxTimePerFrame:uint = 5):void {
            ...
            if (Boolean(callback)) {
                ...
                if (length < 24576) {
                    ...
                    for (var i:uint = 0, l:uint = 24576 - length; i < l; i++) {
                        _cachedWave.writeFloat(0.0);
                    }
                }
            }
        }
         ...
         * @param    maxTimePerFrame        Maximum time in milliseconds the caching will use per frame
         */
        public function cacheMutations(mutationsNum:uint, callback:Function = null, maxTimePerFrame:uint = 5,
                mutationAmount:Number = 0.05):void {
            _playMutated = true;
            stop();
            ...
            if (callback == null){
                throw new ArgumentError("Callback has to be passed");
            }
         ...
         */
        private function synthWave(buffer:ByteArray, length:uint, waveData:Boolean = false, sampleRate:uint = 44100,
                bitDepth:uint = 16):Boolean {
            _finished = false;
            ...
            for (var i:uint = 0; i < length; i++) {
                ...
                for (var j:int = 0; j < 8; j++) {
                    ...
                    if (_phase >= _periodTemp) {
                        ...
                        // Generates new net.retrocade.random noise for this period
                        if (_waveType == 3) {
                            for (var n:uint = 0; n < 32; n++) {
                    ...
                    switch (_waveType) {
                        ...
                        case 3: // Noise
                        {
                            _sample = _noiseBuffer[uint(_phase * 32 / int(_periodTemp)) % 32];
                            break;
                        }
                    }
