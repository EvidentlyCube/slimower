package game.objects {
	...
	public class TMonsterSlimeGreen extends TMonster {
		...
		public function TMonsterSlimeGreen(x:uint, y:uint, speed:Number) {
			if (!framesWalk) {
				framesWalk = [];
				framesWalk[0] = RetrocamelBitmapManager.getBDExt(_gfx_, 0, 0, 16, 16);
				framesWalk[1] = RetrocamelBitmapManager.getBDExt(_gfx_, 16, 0, 16, 16);
				framesWalk[2] = RetrocamelBitmapManager.getBDExt(_gfx_, 32, 0, 16, 16);
				framesWalk[3] = RetrocamelBitmapManager.getBDExt(_gfx_, 16, 0, 16, 16);

				framesDamage = [];
				framesDamage[0] = RetrocamelBitmapManager.getBDSpecial(_gfx_, 0, 0, 16, 16, false, 0xFFFFFF);
				framesDamage[1] = RetrocamelBitmapManager.getBDSpecial(_gfx_, 16, 0, 16, 16, false, 0xFFFFFF);
				framesDamage[2] = RetrocamelBitmapManager.getBDSpecial(_gfx_, 32, 0, 16, 16, false, 0xFFFFFF);
				framesDamage[3] = RetrocamelBitmapManager.getBDSpecial(_gfx_, 16, 0, 16, 16, false, 0xFFFFFF);
			}
		...
		private function makeBullet(x:Number, y:Number, dir:int = 0):void {
			...
			new TSlimeBullet(x, y, Math.random() * 1 * dir, -Math.random() * 1.5 - 2.2, 0);
		}
	}
}